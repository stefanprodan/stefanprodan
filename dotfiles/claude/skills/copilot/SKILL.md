...
name: copilot
description: >
  Delegate a task to GitHub Copilot CLI (GPT-6) by running
...
```shell
copilot -C <working directory> --model gpt-6-astra --effort high --allow-all --no-color --log-level none -p "$(cat <<'TASK'
<task prompt>
TASK
